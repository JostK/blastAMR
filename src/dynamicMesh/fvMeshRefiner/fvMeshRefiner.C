...
bool Foam::fvMeshRefiner::balance()
{
    ...
    if (canBalance(true))
    {
        ...
        Info << "HALLO1 " << mesh_.foundObject<volScalarField::Internal>("V0_Old") << endl;
    
        if (mesh_.V0Ptr_)
        {
            V0OldPtr_ = mesh_.V0Ptr_;
            mesh_.objectRegistry::store(V0OldPtr_, "V0_Old");
            mesh_.V0Ptr_ = nullptr;
        }
        if (mesh_.V00Ptr_)
        {
            V00OldPtr_ = mesh_.V00Ptr_;
            mesh_.objectRegistry::store(V00OldPtr_, "V00_Old");
            mesh_.V00Ptr_ = nullptr;
        }
        Info << "HALLO2 " << mesh_.foundObject<volScalarField::Internal>("V0_Old") << endl;
