...
void kEpsilonPhitFBlast<BasicTurbulenceModel>::correctNut()
{
    // (LUU:p. 173)
...
template<class BasicTurbulenceModel>
tmp<volScalarField> kEpsilonPhitFBlast<BasicTurbulenceModel>::Ts() const
{
    // (LUU:Eq. 7)
    return
        max
...
template<class BasicTurbulenceModel>
tmp<volScalarField> kEpsilonPhitFBlast<BasicTurbulenceModel>::Ls() const
{
    // (LUU:Eq. 7)
    return
        CL_*max
...
template<class BasicTurbulenceModel>
kEpsilonPhitFBlast<BasicTurbulenceModel>::kEpsilonPhitFBlast
(
...
//     ),
    ...
    fMin_(dimensionedScalar("fMin", f_.dimensions(), VSMALL)),
    TMin_(dimensionedScalar("TMin", dimTime, VSMALL)),
    L2Min_(dimensionedScalar("L2Min", sqr(dimLength), VSMALL))
{
    bound(k_, this->kMin_);
...
template<class BasicTurbulenceModel>
bool kEpsilonPhitFBlast<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void kEpsilonPhitFBlast<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
